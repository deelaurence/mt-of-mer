...
const ArticlesAll = ({allArticles}) => {
  ...
  const [sliced, setSliced] =useState(allArticles.slice(0,10))
  ...
  const changePage=(e)=>{
   ...
   setCurrentPage(Number(e.target.innerHTML))
    ...
    console.log(pageStart,pageEnd)
      setSliced(allArticles.slice(pageStart, pageEnd))
    }
    useEffect(()=>{
    const firstPage=setTimeout(() => {
       setSliced(allArticles.slice(0,10))
...
//   }, [currentLocation, location])
  return (
    ...
    <section className='bg-darkShade'>
      ...
      {allArticles.length>0&&<div className={currentPage==1?"font-bold":"italic opacity-[.85]"} onClick={changePage}>1</div>}
      {allArticles.length>10&&<div className={currentPage==2?"font-bold":"italic opacity-[.85]"} onClick={changePage} >2</div>}
      {allArticles.length>20&&<div className={currentPage==3?"font-bold":"italic opacity-[.85]"} onClick={changePage} >3</div>}
      {allArticles.length>30&&<div className={currentPage==4?"font-bold":"italic opacity-[.85]"} onClick={changePage} >4</div>}
      {allArticles.length>40&&<div className={currentPage==5?"font-bold":"italic opacity-[.85]"} onClick={changePage} >5</div>}
      {allArticles.length>50&&<div className={currentPage==6?"font-bold":"italic opacity-[.85]"} onClick={changePage} >6</div>}
      {allArticles.length>60&&<div className={currentPage==7?"font-bold":"italic opacity-[.85]"} onClick={changePage} >7</div>}
      {allArticles.length>70&&<div className={currentPage==8?"font-bold":"italic opacity-[.85]"} onClick={changePage} >8</div>}
      {allArticles.length>80&&<div className={currentPage==9?"font-bold":"italic opacity-[.85]"} onClick={changePage} >9</div>}
      {allArticles.length>90&&<div className={currentPage==10?"font-bold":"italic opacity-[.85]"} onClick={changePage} >10</div>}
      {allArticles.length>100&&<div className={currentPage==11?"font-bold":"italic opacity-[.85]"} onClick={changePage} >11</div>}
      {allArticles.length>110&&<div className={currentPage==12?"font-bold":"italic opacity-[.85]"} onClick={changePage} >12</div>}
      </section>
    </main>
}
    </>
